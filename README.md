...
- 首版回归集包含 3 道真实问题。上一轮完整生产调用曾达到 3/3；人工复读发现第三题存在跨制度扩张后，已新增确定性拦截和回归测试。最新代码仍需在 DeepSeek 服务恢复容量后重新完成真实调用验收，旧结果不作为最新版最终结论。
...
当前本地回归包括7项检索测试、28项受控智能体测试和47项文档处理测试。Node 测试覆盖索引一致性、中文 BM25、向量行号、RRF、工具输入校验、规划失败降级、第二轮检索、循环上限、无密钥降级、幻觉引用拒绝、逐字引文、数字与日期防篡改、URL 回填、未来规则、跨主体制度拦截和审查后修订。Python 测试覆盖文档解析、结构化与 Chunk 生成；三组测试是不同层次，不能用其中一组代替另一组。
