...
这是一个面向证券、基金、期货及场外衍生品业务的法规问答项目。系统不依赖大模型记忆直接回答，而是先将监管原件整理为可追溯的知识库，再通过混合检索获取相关条文，最后仅依据本次检索到的原文生成回答，并展示法规名称、文号、条款范围和官网链接。
...
以上数字直接读取当前 `data/processed/build_manifest.json` 和 `data/index/manifest.json`。
...
Chunk 不按固定字数截断，优先在完整条款、完整分项、完整句子和表格行之间切分。处理时重点避免：
...
结构化正文和 Chunk 可以在[法规知识库小工具](deliverables/场外衍生品法规知识库_20260727.html)中逐份查看；解析、增量构建和评测证据见[项目迭代记录](docs/history/项目迭代记录.md)。
...
确认后，Agent 使用完整问题调用混合检索，不预先拆分为针对单个问题的固定关键词。Agent 根据第一轮证据判断是否需要补检索；如需补充，则保留第一轮仍然相关的证据并合并第二轮结果。

如果原文只规定了一般资格、风控或信息披露要求，却没有直接写明用户问的具体产品“可以”或“不可以”，系统会回答“现有证据不足以形成确定结论”，并列出已找到的制度边界和仍缺少的直接依据。该结果用于保留法规适用边界，不代表流程异常。
...
这三道题不对应专门的运行时规则，也没有预先指定召回 Chunk。它们用于检验同一套流程在不同证据强度下的表现：有直接条文时给出明确回答，只有相邻制度时说明适用边界，无法确定时不补出确定性结论。
...
该机制用于在法规检索之外保存经确认的业务知识，但不替代法规原文或法律判断。
