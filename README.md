...
- 108 份正式监管原件，位于 `data/raw/监管文件/`；另保留1条错名重复原件的排除记录。
...
- 旧 BM25/向量索引已清除；检索与问答层尚未基于新 Chunk 重建。
...
- 当前已完成 108 份原件的结构化转换、Chunk 切分和独立复核，但尚未重建 BM25 和向量索引，API 会返回索引未就绪。
- 检索、RRF 融合、API 和前端架构保留现有实现，后续需以 1,219 个新 Chunk 为输入重建索引并做回归验证。
