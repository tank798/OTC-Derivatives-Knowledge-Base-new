...
test("无向量重建在写索引前拒绝不兼容的Chunk行号或文本", () => {
  const corpus = [
    {
      chunk_id: "chunk-a",
      document_title: "测试法规",
  ...
  const validMetadata = {
    row_chunk_ids: ["chunk-a"],
  };
  const fingerprint = validateExistingVectorCompatibility(corpus, validMetadata);
  assert.match(fingerprint, /^[0-9a-f]{64}$/);
  assert.throws(
    () => validateExistingVectorCompatibility(corpus, { row_chunk_ids: ["chunk-b"] }),
    /Chunk行号已变化/,
  );
  assert.throws(
    () => validateExistingVectorCompatibility(corpus, {
      row_chunk_ids: ["chunk-a"],
      embedding_input_sha256: "0".repeat(64),
    }),
    /Embedding输入已变化/,
  );
});

test("检索语料与manifest数量一致且不含占位清单", { skip: !HAS_INDEX && "尚未基于正式法规重建索引" }, () => {
  ...
  assert.equal(
    new Set(corpus.map((row) => row.document_id)).size,
...
test("向量文件与Chunk行号严格对应", { skip: !HAS_INDEX && "尚未基于正式法规重建索引" }, () => {
...
test("增量向量审计证明复用行字节不变", { skip: !HAS_INDEX && "尚未基于正式法规重建索引" }, () => {
  const { corpus, vectorMetadata } = loadIndexArtifacts();
...
test("中文BM25包含专业词且不使用字段权重", { skip: !HAS_INDEX && "尚未基于正式法规重建索引" }, () => {
...
test("输入准确法规名称能够召回对应法规", { skip: !HAS_INDEX && "尚未基于正式法规重建索引" }, () => {
...
test("输入文号能够召回对应法规", { skip: !HAS_INDEX && "尚未基于正式法规重建索引" }, () => {
  const { corpus, bm25 } = loadIndexArtifacts();
