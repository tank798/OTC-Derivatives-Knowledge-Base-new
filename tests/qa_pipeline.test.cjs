...
const { RetrievalService } = require("../apps/api/dist/apps/api/src/modules/retrieval/retrieval.service.js");
...
function makeHarness(options = {}) {
  ...
  const llm = {
    ...
    chat: async (system, user, timeout, options) => {
      calls.push({ system, user, timeout, options });
...
test("规划识别的交易对手会进入混合检索范围", async () => {
  const planned = plan({ counterparties: ["商业银行"], subjects: ["上市公司"] });
  const harness = makeHarness({ plans: [planned] });
  await harness.service.answer("上市公司与商业银行的衍生品交易有何限制", { debug: true });
  assert.deepEqual(harness.toolInputs[0].counterparties, ["商业银行"]);
  assert.ok(harness.toolInputs[0].queries.some((query) => query.startsWith("商业银行 ")));
});

test("有时间范围的法规问题会同时检索现行与尚未施行规则", async () => {
  const harness = makeHarness({ plans: [plan({ timeScope: "截至当前日期" })] });
  await harness.service.answer("某业务当前能否开展", { debug: true });
  assert.ok(harness.toolInputs[0].queries.some((query) => /现行有效/.test(query) && /尚未施行/.test(query)));
});

test("多查询合并时为后置直接规则查询保留通道名额", () => {
  const retrieval = new RetrievalService();
  const broad = Array.from({ length: 30 }, (_, index) => ({
    chunk_id: `broad_${index + 1}`, index, rank: index + 1, bm25: 100 - index,
  }));
  const direct = Array.from({ length: 5 }, (_, index) => ({
    chunk_id: `direct_${index + 1}`, index: 100 + index, rank: index + 1, bm25: 1 - index / 10,
  }));
  const merged = retrieval.mergeChannel([broad, direct], "bm25");
  assert.deepEqual(merged.slice(0, 4).map((hit) => hit.chunk_id), ["broad_1", "direct_1", "broad_2", "direct_2"]);
});

test("检索范围会将历史口语对手名称扩展为法规正式名称", () => {
  const retrieval = new RetrievalService();
  const expanded = retrieval.expandScopeTerms(["期货公司风险管理子公司"]);
  assert.ok(expanded.includes("期货风险管理公司"));
});

test("模型生成的每条检索式保持原样并另加范围查询", () => {
  const retrieval = new RetrievalService();
  const input = {
    queries: ["某直接规则 尚未施行"], subQuestion: "某业务能否开展",
    subjects: ["某主体"], productTypes: ["某产品"], counterparties: ["某对手"],
    timeScope: "当前", requiredEvidence: [], topK: 10,
  };
  const queries = retrieval.buildChannelQueries(input);
  assert.equal(queries.keywordQueries[0], "某直接规则 尚未施行");
  assert.ok(queries.keywordQueries.some((query) => /某主体.*某产品.*某对手/.test(query)));
});

test("非直接证据在调用修订模型前先保守降为不能确认", async () => {
  const harness = makeHarness({
    assessments: [assessment({ evidenceLevel: "INFERRED" })],
    answers: [modelAnswer(), modelAnswer({
      directAnswer: "不能确认", conclusionLevel: "证据不足",
      conclusion: "不能确认，现有证据只规定一般条件。",
      scope: { subject: "证券公司", product: "场外期权", counterparty: "", time: "当前", conditions: [] },
      regulatoryBasis: [{ evidenceId: "chunk_1", quoteExact: RULE_TEXT, supports: "只支持一般条件" }],
      confidence: "low",
    })],
  });
  const result = await harness.service.answer("证券公司可以开展某具体结构吗", { debug: true });
  assert.equal(result.answer.directAnswer, "不能确认");
  assert.equal(result.agentTrace.repairCount, 0);
});

test("条文含量化例外时不得只写满足条件除外", () => {
  const rule = "投资比例不得超过净资产的 25%,单个投资者投资金额不低于 1000万元的封闭式产品除外。";
  const answer = draft({
    conclusion: "投资比例不得超过净资产的25%，满足一定条件的产品除外。",
    regulatoryBasis: [{ evidenceId: "chunk_1", title: "", publisher: "", url: "", articleNo: "", excerpt: rule, quoteExact: rule, requirement: "比例及例外", status: "" }],
  });
  const checked = validator.validate(answer, [hit({ text: rule, excerpt: rule })], analyzer.analyze("某投资比例有明确规定吗"), "DIRECT");
  assert.equal(checked.citationValidation.passed, false);
  assert.match(checked.citationValidation.issues.join("\n"), /1000万元/);
});

test("边界旁证不把其他制度的例外强加给不能确认答案", () => {
  const rule = "某类产品投资比例不得超过净资产的 25%,单个投资者投资金额不低于 1000万元的封闭式产品除外。";
  const answer = draft({
    directAnswer: "不能确认",
    conclusionLevel: "证据不足",
    conclusion: "现有条文只规范另一类主体的投资比例，未直接规定当前主体能否发行该结构。",
    conclusionLabel: "需人工合规复核",
    scope: { subject: "发行机构", product: "某结构产品", counterparty: "", time: "当前", conditions: [] },
    regulatoryBasis: [{
      evidenceId: "chunk_1", title: "", publisher: "", url: "", articleNo: "",
      excerpt: rule, quoteExact: rule, requirement: "仅说明另一主体的规则边界",
      supportRole: "BOUNDARY_ONLY", status: "",
    }],
  });
  const checked = validator.validate(answer, [hit({ text: rule, excerpt: rule })], analyzer.analyze("当前主体能否发行某结构产品"), "INFERRED");
  assert.equal(checked.citationValidation.passed, true);
});

test("确定性结论不能只依赖边界旁证", () => {
  const answer = draft({
    regulatoryBasis: [{ ...draft().regulatoryBasis[0], supportRole: "BOUNDARY_ONLY" }],
  });
  const checked = validator.validate(answer, [hit()], analyzer.analyze("证券公司可以开展场外期权吗"), "DIRECT");
  assert.equal(checked.citationValidation.passed, false);
  assert.match(checked.citationValidation.issues.join("\n"), /旁证/);
});

test("最终证据上下文最多保留 10 个 Chunk", async () => {
  const manyHits = Array.from({ length: 15 }, (_, index) => hit({
    id: `chunk_${index + 1}`,
    chunkId: `chunk_${index + 1}`,
    score: 1 - index / 100,
  }));
  const harness = makeHarness({ hitRounds: [manyHits] });
  const result = await harness.service.answer("证券公司可以开展场外期权吗", { debug: true });
  assert.equal(result.hits.length, 10);
  assert.deepEqual(result.hits.map((item) => item.id), manyHits.slice(0, 10).map((item) => item.id));
});

test("只有低风险检索规划使用 Flash 非思考模式", async () => {
  const harness = makeHarness();
  await harness.service.answer("证券公司可以开展场外期权吗", { debug: true });
  const plannerCall = harness.calls.find((call) => call.system === "PLANNER" && call.user.includes("mode: PLAN"));
  assert.deepEqual(plannerCall.options, { tier: "fast", thinking: "disabled" });
  for (const call of harness.calls.filter((item) => item !== plannerCall)) {
    assert.deepEqual(call.options, { tier: "default" });
  }
});
...
test("DeepSeek 瞬时过载会有限重试，且可显式路由到 Flash", async () => {
  ...
  try {
    const llm = new LlmService({
      get: (key) => ({ LLM_API_KEY: "test-only-key", LLM_MODEL: "deepseek-v4-pro", LLM_FAST_MODEL: "deepseek-v4-flash", LLM_BASE_URL: "https://example.test" })[key],
    });
    assert.equal(await llm.chat("system", "user", 1000), "ok");
    assert.equal(requests.length, 2);
    assert.ok(requests.every((request) => request.model === "deepseek-v4-pro"));
    assert.equal(await llm.chat("system", "user", 1000, { tier: "fast", thinking: "disabled" }), "ok");
    assert.equal(requests[2].model, "deepseek-v4-flash");
...
test("证据缺口字段偶发输出对象时可归一为文本", async () => {
  const harness = makeHarness({ assessments: [assessment({
    sufficient: false,
    answerability: "UNCERTAIN",
    evidenceLevel: "INSUFFICIENT",
    missingSubQuestions: [{ question: "缺少直接禁止或许可规则" }],
    missingEvidenceTypes: [{ description: "主体和产品同时明确的直接条文" }],
    followUpQueries: [],
  })] });
  const result = await harness.service.answer("某主体能否开展某场外衍生品", { debug: true });
  assert.deepEqual(result.evidenceAssessment.missingSubQuestions, ["缺少直接禁止或许可规则"]);
  assert.deepEqual(result.evidenceAssessment.missingEvidenceTypes, ["主体和产品同时明确的直接条文"]);
});

test("以未禁止或间接推断为理由时不得自报为直接证据", async () => {
  const harness = makeHarness({
    assessments: [assessment({
      sufficient: true, answerability: "YES", evidenceLevel: "DIRECT",
      reasonSummary: "规则未明确禁止该结构，可据此推断允许。",
    })],
    answers: [modelAnswer(), modelAnswer({
      directAnswer: "不能确认", conclusionLevel: "证据不足",
      conclusion: "不能确认，现有证据未直接规定该结构。", confidence: "low",
    })],
  });
  const result = await harness.service.answer("某具体结构是否可以开展", { debug: true });
  assert.equal(result.evidenceAssessment.evidenceLevel, "INFERRED");
  assert.equal(result.answer.directAnswer, "不能确认");
});

test("仅将必要条件表述为满足即可时应降为推导证据", async () => {
  const harness = makeHarness({
    assessments: [assessment({
      sufficient: true, answerability: "YES", evidenceLevel: "DIRECT",
      reasonSummary: "条文规定了亏损上限，若能满足该条件则可发行。",
    })],
    answers: [modelAnswer(), modelAnswer({
      directAnswer: "不能确认", conclusionLevel: "证据不足",
      conclusion: "不能确认，必要条件不等于对具体结构的直接许可。", confidence: "low",
    })],
  });
  const result = await harness.service.answer("某具体结构能否发行", { debug: true });
  assert.equal(result.evidenceAssessment.evidenceLevel, "INFERRED");
  assert.equal(result.answer.directAnswer, "不能确认");
});

test("证据判断已识别未来规则时最终回答不得遗漏", () => {
  const futureText = "某业务自明年起不得开展，监管机构另有规定的除外。";
  const current = hit();
  const future = hit({ id: "chunk_future", chunkId: "chunk_future", text: futureText, excerpt: futureText, status: "已公布、尚未施行" });
  const checked = validator.validate(
    draft(), [current, future], analyzer.analyze("当前能否开展"), "DIRECT",
    "未来规则尚未施行，生效后将禁止。",
  );
  assert.equal(checked.citationValidation.passed, false);
  assert.match(checked.citationValidation.issues.join("\n"), /chunk_future/);
...
test("多子问题证据按子问题均衡保留，后置直接规则不会被前一批挤出", async () => {
  ...
  assert.ok(secondQuestionHits.length >= 5);
...
test("未来规则的非量化但书从已验证引文中确定性保留", () => {
  const exceptionText = "上市公司不得达成相关衍生品交易,法律、行政法规、中国证监会另有规定的除外。";
  const futureHit = hit({ text: exceptionText, excerpt: exceptionText, status: "已公布、尚未施行" });
  const checked = validator.validate(draft({
    directAnswer: "不能确认", conclusionLevel: "证据不足",
    conclusion: "该规则尚未施行，未来生效后将作出禁止性规定。",
    conclusionLabel: "需人工合规复核",
    regulatoryBasis: [{ ...draft().regulatoryBasis[0], excerpt: exceptionText, quoteExact: exceptionText, supportRole: "FUTURE_RULE" }],
  }), [futureHit], analyzer.analyze("上市公司未来能否开展相关衍生品"), "INSUFFICIENT");
  assert.equal(checked.citationValidation.passed, true);
  assert.ok(checked.restrictions.some((item) => item.includes("另有规定的除外")));
});

test("同一 Chunk 的不同逐字引文不会被错误合并", () => {
  const first = "第六条 发行机构应当具备业务资格。";
  const second = "第九条 产品损失不得超过约定范围。";
  const evidence = hit({ text: `${first}\n${second}`, excerpt: `${first}\n${second}` });
  const checked = validator.validate(draft({
    regulatoryBasis: [
      { ...draft().regulatoryBasis[0], excerpt: first, quoteExact: first },
      { ...draft().regulatoryBasis[0], excerpt: second, quoteExact: second },
    ],
  }), [evidence], analyzer.analyze("发行机构有哪些条件"), "DIRECT");
  assert.equal(checked.citationValidation.passed, true);
  assert.equal(checked.regulatoryBasis.length, 2);
});

test("局部禁止不会被程序硬编码扩大为全面否定", () => {
...
test("通用 Prompt 保留跨制度边界，但不内置三道评测题", () => {
  const prompts = [
    readFileSync("packages/prompts/agent/retrieval-planner.md", "utf8"),
    readFileSync("packages/prompts/agent/evidence-answer.md", "utf8"),
    readFileSync("packages/prompts/agent/answer-reviewer.md", "utf8"),
  ].join("\n");
  assert.match(prompts, /跨制度|其他监管制度/);
  assert.doesNotMatch(prompts, /券商收益凭证可以做雪球吗|上市公司可以做挂钩自己股票|私募产品投资雪球比例/);
});
...
test("证据不足可按通用审查意见修订，不注入题目专项指令", async () => {
  ...
  const harness = makeHarness({
    ...
    reviews: [
      { verdict: "REPAIR", issues: [{ type: "SCOPE", severity: "MAJOR", statement: "一般条件不足以支持具体产品许可", evidenceId: "chunk_1", reason: "改为不能确认并说明现有规则边界", action: "修订" }], repairInstructions: ["改为不能确认并说明现有规则边界"], missingEvidence: [], followUpQueries: [] },
      { verdict: "PASS", issues: [], repairInstructions: [], missingEvidence: [], followUpQueries: [] },
    ],
  });
  const result = await harness.service.answer("券商收益凭证可以做雪球吗", { debug: true });
  const answerCalls = harness.calls.filter((call) => call.system === "ANSWER");
  assert.equal(answerCalls.length, 2);
  assert.doesNotMatch(answerCalls[1].user, /mandatory_/);
  assert.match(answerCalls[1].user, /改为不能确认并说明现有规则边界/);
  assert.equal(result.answer.directAnswer, "不能确认");
...
test("API key 不出现在受控智能体代码、Prompt 或 Git 差异中", () => {
  ...
  const diff = [
    execFileSync("git", ["diff", "--", ...paths], { encoding: "utf8" }),
    execFileSync("git", ["diff", "--cached", "--", ...paths], { encoding: "utf8" }),
  ].join("\n");
