...
test("上市公司自身股票结论实质为不能笼统认定时稳定输出否", () => {
  const query = analyzer.analyze("上市公司可以做挂钩自己本身标的的场外衍生品吗");
  const currentRule = hit({
    id: "chunk_current",
    chunkId: "chunk_current",
  ...
  const checked = validator.validate(answer({
    directAnswer: "不能确认",
    conclusion: "不能笼统确认上市公司可以做挂钩自身股票的场外衍生品，现行禁止期货风险管理公司路径，未来规则尚未施行。",
    conclusionLabel: "需人工合规复核",
    regulatoryBasis: [
      { ...answer().regulatoryBasis[0], evidenceId: "chunk_current" },
      { ...answer().regulatoryBasis[0], evidenceId: "chunk_future" },
    ],
  }), [currentRule, futureRule], query);
  assert.equal(checked.citationValidation.passed, true);
  assert.equal(checked.directAnswer, "否");
  assert.equal(checked.conclusionLabel, "不可做");
});
