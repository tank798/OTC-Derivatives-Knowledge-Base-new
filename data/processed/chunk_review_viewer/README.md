...
- `chunk_review.html`：直接从正式 `all_chunks.jsonl` 生成，内嵌 108 份法规和 1,221 个 Chunk。
...
总目录生成器从 `data/index/document_metadata.jsonl` 取数；Chunk 查看器从唯一正式 Chunk 集 `all_chunks.jsonl` 取数。两者都只将展示所需字段写入最终 HTML。
